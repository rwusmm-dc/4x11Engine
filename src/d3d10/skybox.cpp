...
ComPtr<ID3D10RasterizerState>   g_RSState;
...
static bool CompileShaders()
{
    ...
    ComPtr<ID3D10Blob> vsBlob, psBlob, err;

    HRESULT hr = D3DCompile(g_HLSL, strlen(g_HLSL), "sky", nullptr, nullptr,
                            "VSMain", "vs_4_0", flags, 0, vsBlob.addr(), err.addr());
    if (FAILED(hr)) {
        if (err) MessageBoxA(nullptr, (char*)err->GetBufferPointer(), "Sky VS Error (D3D10)", MB_OK);
        return false;
    }
...
static void CreateStates()
{
    ...
    ThrowIfFailed(dev->CreateDepthStencilState(&dsd, g_DSState.addr()), "Sky DS failed (D3D10)");
    ...
    ThrowIfFailed(dev->CreateRasterizerState(&rd, g_RSState.addr()), "Sky RS failed (D3D10)");
}
...
static void CreateConstantBuffer()
{
    ...
    ThrowIfFailed(dev->CreateBuffer(&bd, nullptr, g_CB.addr()), "Sky CB failed (D3D10)");
}
...
void Draw(const float* viewMatrix, const float* projMatrix, float aspectRatio, float totalTime)
{
    ...
    dev->UpdateSubresource(g_CB.get(), 0, nullptr, &cb, 0, 0);

    dev->OMSetDepthStencilState(g_DSState.get(), 0);
    dev->RSSetState(g_RSState.get());
    ...
    ID3D10Buffer* cbs[] = { g_CB.get() };
    dev->VSSetConstantBuffers(0, 1, cbs);
    dev->PSSetConstantBuffers(0, 1, cbs);
